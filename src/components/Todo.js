...
const Todo = () => {
  ...
  const addHandler = () => {
    if (items === "") {
      ...
      dispatch(addTodo(items), setItem(""));
    }
  };
    ...
    <>
      <div className="my-56 px-0">
        ...
        <div className="text-2xl  mx-9 font-bold bg-white 810 my-20 min w-96 rounded-md  border-solid border-2 border-light-blue-900">
          <input
            className="focus:outline-none focus-visible:ring  active:bg-none  placeholder-shown:border-gray-500 required:border-red-500  py-2 px-4 text-orange-500 w-90 focus:border-none "
            type="text"
        ...
        <div className="text-3xl my-20 flex items-center  mx-20">
          <button
            className="px-20 py-3 font bg-teal-400 text-pink-600 rounded-md"
            onClick={() => {
              dispatch(removeTodo());
            }}
          >
